...
assert(A.named[0].ok && A.named[0].name === 'bSpeedy/b McGee-the-', 'name is stripped of markup, whitespace-collapsed and capped at 20 chars: ' + JSON.stringify(A.named[0].name))
...
// Profanity filter: plain, leetspeak/spaced evasions, and no false positives on
// innocent names that merely contain a short blocked word.
send(A, { t: 'name', name: 'shithead' })
assert(await waitFor(() => A.named.length === 4, 2000, 'profane name'), 'profane name answered')
assert(A.named[3].ok === false && A.named[3].name === 'Speedy' && /clean/i.test(A.named[3].why), 'profane name rejected, previous name kept: ' + JSON.stringify(A.named[3]))
send(A, { t: 'name', name: 'S h 1 t' })
assert(await waitFor(() => A.named.length === 5, 2000, 'leet name'), 'leetspeak name answered')
assert(A.named[4].ok === false && A.named[4].name === 'Speedy', 'spaced leetspeak evasion rejected, previous name kept')
send(A, { t: 'name', name: 'Classy Bassist' })
assert(await waitFor(() => A.named.length === 6, 2000, 'classy name'), 'Classy Bassist answered')
assert(A.named[5].ok && A.named[5].name === 'Classy Bassist', '"ass" inside ordinary words is allowed')
send(A, { t: 'name', name: 'Cassidy Sexton' })
assert(await waitFor(() => A.named.length === 7, 2000, 'sexton name'), 'Cassidy Sexton answered')
assert(A.named[6].ok && A.named[6].name === 'Cassidy Sexton', '"sex" inside a surname is allowed')
send(A, { t: 'name', name: 'Max Power Twenty Chr' })
assert(await waitFor(() => A.named.length === 8, 2000, '20-char name'), '20-char name answered')
assert(A.named[7].ok && A.named[7].name === 'Max Power Twenty Chr', 'a 20-character name with spaces is accepted unchanged')
send(A, { t: 'name', name: 'Max Power Twenty Five' + 'XYZW' })
assert(await waitFor(() => A.named.length === 9, 2000, '25-char name'), '25-char name answered')
assert(A.named[8].ok && A.named[8].name === 'Max Power Twenty Fiv', '25-character name is cut to 20: ' + JSON.stringify(A.named[8].name))
send(A, { t: 'name', name: 'Speedy' })
assert(await waitFor(() => A.named.length === 10, 2000, 'back to Speedy'), 'rename back to Speedy answered')
assert(A.named[9].ok && A.named[9].name === 'Speedy', 'back to Speedy for the rest of the run')
...
assert(await waitFor(() => A.named.length === 11 && A.named[10].ok, 2000, 'lobby rename'), 'a racer can still rename while the lobby is open')
await sleep(150)
...
assert(await waitFor(() => A.named.length === 12, 2000, 'locked rename'), 'rename during the race answered')
assert(A.named[11].ok === false && A.named[11].name === 'StillLobby' && carOf(A, A).n === 'StillLobby', 'name is locked once the race starts')
